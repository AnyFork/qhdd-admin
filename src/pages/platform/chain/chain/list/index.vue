<template>
    <n-space align="center" justify="space-between" class="mb-2">
        <n-form :show-feedback="false" :size="size" inline :model="searchForm" label-placement="left" class="justify-end">
            <n-button type="primary" @click="createModal = true" class="mr-6">新增连锁店</n-button>
    ...
    <!--创建连锁店-->
    <CreateChain v-if="createModal && chainAdmin" v-model:open="createModal" :admin="chainAdmin" @refresh="getChainListInfo"></CreateChain>
    <!--编辑连锁店 -->
    <UpdateChain v-if="updateModal && rowNode && chainAdmin" v-model:open="updateModal" :admin="chainAdmin" v-model:node="rowNode" @refresh="getChainListInfo"></UpdateChain>
</template>
<script setup lang="ts">
const size = ref<'small' | 'medium' | 'large'>('medium')
const striped = ref(true)
const { height } = useWindowSize()
const { getChainListInfo, searchForm, tableData, loading, pagination, columns, rowNode, createModal, updateModal } = useChain()
