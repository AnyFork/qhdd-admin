<template>
    <n-space align="center" justify="space-between" class="mb-2">
        <n-form :show-feedback="false" :size="size" inline :model="searchForm" label-placement="left" class="justify-end">
            ...
            <n-form-item label="顾客电话">
                <n-input v-model:value="searchForm.mobile" placeholder="请输入顾客电话" clearable class="w-180px" />
            </n-form-item>
    ...
    <!--数据表格 -->
    <n-data-table :striped="striped" remote :size="size" :single-line="false" :columns="columns" :data="tableData" :pagination="pagination" :row-key="(rowData:order.comment) => `${rowData.id}`" :loading="loading" :min-height="tableHeight" :max-height="tableHeight" />
</template>
<script setup lang="ts">
const size = ref<'small' | 'medium' | 'large'>('medium')
const striped = ref(true)
const { height } = useWindowSize()
const { getCustomerListInfo, pagination, tableData, loading, columns, searchForm } = useCustomer()
// 表格高度
