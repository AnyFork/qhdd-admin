<template>
    <n-spin size="large" :show="loading" description="Loading">
        <n-flex align="center" class="pb-2">
            <n-form-item label="店铺名称" label-placement="left" :show-feedback="false">
                <n-select v-model:value="sid" :options="allStore" placeholder="请选择店铺" clearable class="w-260px" />
            </n-form-item>
            <div style="display: flex; justify-content: flex-end">
                <n-button round type="primary" class="w-100px" @click="searchAllList">
                    <template #icon>
        ...
        <div class="flex mt-4 items-center text-18px font-600 before:content-[''] before:w-5px before:h-20px before:bg-primary before:inline-flex before:mr-2">历史经营数据</div>
        <!-- 单选按钮 -->
...
const radioClick = (name: number) => {
    ...
    if (name == 1) {
        ...
        historyStat(statDayStart.value, statDayEnd.value, sid.value)
    }
    if (name == 2) {
...
const confirm = (_value: number | [number, number] | null, formattedValue: [string, string] | null) => {
    if (formattedValue) {
        historyStat(Number(formattedValue[0]), Number(formattedValue[1]), sid.value)
    }
}
