<template>
    <n-drawer v-model:show="open" :width="800" placement="right" :mask-closable="false" :close-on-esc="false">
        <n-drawer-content title="创建公告" closable>
...
const { formRef, rules, moduleValue, addNoticeInfoInfo, loading, message } = useNotice()
...
const submitCallback = (e: MouseEvent) => {
    ...
    formRef.value?.validate(async (errors: any) => {
        if (!errors) {
            await addNoticeInfoInfo(moduleValue.value)
            open.value = false
            emit('refresh')
        } else {
            console.log(errors)
