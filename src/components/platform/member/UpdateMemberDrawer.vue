...
const props = defineProps<{ rowNode: member.level }>()
const emit = defineEmits<{ refresh: [] }>()
const { formRef, moduleValue, rules, updateMemberInfo, loading, message } = usePlatformMember()
...
const submitCallback = (e: MouseEvent) => {
    ...
    formRef.value?.validate(async (errors) => {
        if (!errors) {
            await updateMemberInfo(moduleValue)
            active.value = false
            emit('refresh')
            // 清空表单校验
...
onMounted(() => {
    ...
    moduleValue.level = props.rowNode.level
    moduleValue.conditionType = props.rowNode.conditionType
    moduleValue.conditionDesp = props.rowNode.conditionDesp
    moduleValue.conditionValue = props.rowNode.conditionValue
    moduleValue.redpacketAmount = props.rowNode.redpacketAmount
    moduleValue.redpacketCount = props.rowNode.redpacketCount
    moduleValue.redpacketDayLimit = props.rowNode.redpacketDayLimit
    moduleValue.benefitsCount = props.rowNode.benefitsCount
    moduleValue.benefitsDesp = props.rowNode.benefitsDesp
    moduleValue.redpacketUseAmountLimit = props.rowNode.redpacketUseAmountLimit
})
</script>
