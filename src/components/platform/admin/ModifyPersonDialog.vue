...
const { formRef, rules, moduleValue, updateAdminInfo, loading, message } = usePlatformAdmin()
const open = defineModel<boolean>('open')
const { rowNode } = defineProps<{ rowNode: system.adminInfo }>()
...
const submitCallback = (e: MouseEvent) => {
    ...
    formRef.value?.validate(async (errors) => {
        if (!errors) {
            const { id, name, avatar, phone, password } = moduleValue
            await updateAdminInfo({ id, name, avatar, phone, newPassword: password })
            open.value = false
            //清除表单数据
