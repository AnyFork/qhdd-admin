...
const { updateChainAdminInfo, loading, message, rules } = useChainAdmin()
const active = defineModel<boolean>('open')
const model = defineModel<chain.chainAdmin>('node', { required: true })
...
const submitCallback = (e: MouseEvent) => {
    ...
    formRef.value?.validate(async (errors: any) => {
        if (!errors) {
            const { id, realname, mobile, password, status, displayorder } = model.value
            await updateChainAdminInfo({ id, realname, mobile, password, status, displayorder })
            active.value = false
            emit('refresh')
        } else {
            console.log(errors)
