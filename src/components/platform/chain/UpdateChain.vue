<template>
    <n-drawer v-model:show="active" :default-width="500" placement="right" :mask-closable="false" :close-on-esc="false">
        <n-drawer-content title="修改连锁店" closable>
...
const { updateChainInfo, loading, message, rules } = useChain()
defineProps<{ admin: any[] }>()
const active = defineModel<boolean>('open')
const model = defineModel<chain.chainInfo>('node', { required: true })
...
watchEffect(() => {
    if (node.value?.attachment) {
        model.value.logo = node.value?.attachment
    }
})
...
const submitCallback = (e: MouseEvent) => {
    ...
    formRef.value?.validate(async (errors: any) => {
        if (!errors) {
            const { id, title, logo, chainerId, status, displayorder } = model.value
            await updateChainInfo({ id, title, logo, chainerId, status, displayorder })
            active.value = false
            emit('refresh')
        } else {
            console.log(errors)
...
onMounted(() => {
    if (model.value?.logo) {
        //@ts-ignore
        node.value = {
            url: previewUrl + model.value?.logo,
            attachment: model.value?.logo
        }
    }
})
</script>
