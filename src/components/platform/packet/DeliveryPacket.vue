...
import { sendPacket } from '@/service/platform/customer'
import { FormRules } from 'naive-ui'
const showModel = defineModel<boolean>('showModel')
const props = defineProps<{ rowData: userInfo.user }>()
const emit = defineEmits<{ refresh: [] }>()
const modelTitle = computed(() => `给 ${props.rowData.nickname}发送红包`)
...
const submitCallback = (e: MouseEvent) => {
    ...
    formRef.value?.validate(async (errors: any) => {
        if (!errors) {
            try {
                loading.value = true
                const { data } = await sendPacket(packet.value)
                loading.value = false
                if (data.code == 200) {
                    message.success('红包发送成功,请在红包详情里面查看')
