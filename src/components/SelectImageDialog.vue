    ...
    <n-modal v-model:show="open" preset="card" title="选择图片" :mask-closable="false" :close-on-esc="false" class="w-[980px]">
        <n-grid x-gap="10" class="border border-solid border-[#f5f5f5]">
            <n-grid-item span="6">
                <n-flex vertical class="p-2 border-r border-r-solid border-r-[#f5f5f5]">
                    ...
                    <n-spin :show="loading">
                        <n-scrollbar :style="{ height: boxHeight - 100 + 'px' }">
            ...
            <n-grid-item span="18">
                ...
                <div class="p-2">
                    <n-spin size="large" description="Loading" :show="loading">
                        <n-scrollbar v-if="pictures.length > 0" :style="{ height: boxHeight - 100 + 'px' }">
                            <n-grid y-gap="20" x-gap="20" class="border border-solid border-[#f5f5f5] p-2">
                                <n-grid-item span="4" v-for="(item, index) in pictures" :key="'picture' + index" class="flex-col-center">
        ...
        <template #footer>
            <div class="flex justify-end items-center gap-4">
                ...
                <n-button type="primary" @click="selectImage" class="!w-100px">确认</n-button>
            </div>
        </template>
    </n-modal>
</template>
...
const boxHeight = computed(() => height.value - 400)
