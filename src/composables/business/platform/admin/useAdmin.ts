import { deletePlatformAdmin, platformAdminList, updatePlatformAdmin, addPlatformAdmin, platformRoleList } from '@/service/platform/system/admin'
...
export const usePlatformAdmin = () => {
    ...
    const adminList = async () => {
        try {
            loading.value = true
            const { data } = await platformAdminList({ pageNo: pagination.page, pageSize: pagination.pageSize, ...searchForm })
            loading.value = false
            if (data.code == 200) {
    ...
    const updateAdminInfo = async (params: Partial<system.adminInfo>) => {
        try {
            loading.value = true
            const { data } = await updatePlatformAdmin(params)
            loading.value = false
            if (data.code == 200) {
    ...
    const deleteAdminInfo = async (id: number) => {
        try {
            loading.value = true
            const { data } = await deletePlatformAdmin(id)
            loading.value = false
            if (data.code == 200) {
    ...
    const addAdminInfo = async () => {
        try {
            ...
            const { data } = await addPlatformAdmin({ name, phone, avatar, password, roleId })
            loading.value = false
            if (data.code == 200) {
    ...
    const getRoleList = async () => {
        try {
            const { data } = await platformRoleList()
            if (data.code == 200) {
                if (data.data) {
    ...
    return { adminList, updateAdminInfo, pagination, tableData, loading, columns, formRef, moduleValue, rules, addAdminInfo, message, rowNode, CreateShow, modifyShow, getRoleList, rolesSelectOptions, searchForm }
}
