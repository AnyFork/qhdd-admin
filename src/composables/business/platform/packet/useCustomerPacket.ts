import { deletePacket, packetList } from '@/service/platform/customer'
 ...
 * @returns 
 */
export const useCustomerPacket = () => {
    ...
    */
     ...
     * 查询条件
     */
    const searchForm = reactive<Partial<member.redPacket>>({
        type: undefined,
        status: undefined,
        nickname: undefined
    })
    ...
    */
    ...
    const packetListData = async () => {
        try {
            loading.value = true
            const { data } = await packetList({ pageNo: pagination.page, pageSize: pagination.pageSize, ...searchForm })
            console.log(data)
            loading.value = false
            if (data.code == 200) {
    ...
    return {
        packetListData, pagination, tableData, loading, columns, message, EditShow, searchForm, rowNode, packetTypeOption, packetStatusOption
    }
}