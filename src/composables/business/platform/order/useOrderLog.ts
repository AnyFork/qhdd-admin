import { exportOrderInfo, searchAllOrdersInfo } from '@/service/platform/order'
 ...
 * @returns 
 */
export const usePlatformOrderLog = () => {
    ...
    const columns = ref<DataTableColumns<order.orderInfo>>([
        ...
        },
        {
            ...
            render: (_rowData, _index: number) => {
                return chainOptions.value?.find((item: any) => item.value === _rowData.chainid)?.label;
            }
        },
        {
            title: '店铺名称',
