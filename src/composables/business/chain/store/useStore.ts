...
import { addStorePlatform, modifyStoreBatchPlatform, removeStorePlatform, storeListPlatform, storeListPlatformSelect, updateStorePlatform } from '@/service/chain/store'
...
export const useChainStore = () => {
    ...
    const storeListData = async () => {
        try {
            loading.value = true
            console.log({ pageNo: pagination.page, pageSize: pagination.pageSize, ...searchForm })
            const { data } = await storeListPlatform({ pageNo: pagination.page, pageSize: pagination.pageSize, chainid: chainInfo.value.id, ...searchForm })
            loading.value = false
            if (data.code == 200) {
    ...
    return {
        ...
        addStoreInfo, message, CreateShow, searchForm, handleCheck, checkedRowKeysRef, shopOption, deliveryOption,
